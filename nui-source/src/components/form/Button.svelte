<script type="ts">
  export let variant: 'default' | 'cancel' | 'danger' | 'success' | 'transparent' | 'link' | 'blue' | 'amber' = 'default';
  export let size: 'xs' | 'sm' | 'md' | 'lg' = 'md';

  export const buttonVariants = {
    default: 'bg-gray-300 hover:bg-gray-400 text-black dark:hover:brightness-150 dark:bg-secondary dark:text-white border border-gray-400 dark:border-quinary',
  ...
  function isRounded() {
    // check if rounded is contained in the $$restProps.class
    return $$restProps.class?.includes('rounded');
  }
</script>
...
<button
  ...
  disabled:opacity-60 disabled:cursor-not-allowed transition-colors w-full"
>
  <slot />
</button>
