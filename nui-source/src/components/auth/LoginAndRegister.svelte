<script lang="ts">
  ...
  import { authenticate, registerInBrowser } from '@store/auth';
...
<article class="{$$restProps.class} grid">
  {#if page === 'login'}
    <div>
      <hgroup>
        ...
        <img src="https://avatars.githubusercontent.com/u/91481975?s=200&v=4" alt="Logo" class="rounded-full mb-4" />
      </hgroup>
      <Login class="w-full" />
  ...
  {:else}
    <div>
      ...
      <Button on:click={handleOnClick} variant="default" size="md" class="flex items-center justify-center w-full gap-3 mt-5">Go back</Button>
    </div>
  {/if}
  {#if showHero}
    <div />
